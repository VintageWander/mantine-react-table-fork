...
import { Select, MultiSelect, TextInput, useDirection, Tooltip, ActionIcon, CopyButton, UnstyledButton, Highlight, TableTd, Skeleton, TableTr, Collapse, Box, Text, TableTbody, Flex, Button, useMantineTheme, Menu, Switch, TableTh, TableTfoot, Radio, Checkbox, Badge, Alert, Stack, Autocomplete, RangeSlider, Popover, Transition, Indicator, TableThead, Progress, Group, Pagination, Modal, useMantineColorScheme, Table, lighten, darken, LoadingOverlay, Paper } from '@mantine/core';
...
const MRT_ExpandButton = ({ row, table, ...rest }) => {
    ...
    return (jsx(Tooltip, { disabled: !canExpand && !DetailPanel, label: actionIconProps?.title ??
            ...
            }, className: clsx("mrt-expand-button", classes$B.root, classes$B[`root-${rtl ? "rtl" : "ltr"}`], actionIconProps?.className), onClick: handleToggleExpand, title: undefined, children: actionIconProps?.children ?? (jsx(IconChevronDown, { className: clsx("mrt-expand-button-chevron", classes$B.chevron, !canExpand && !renderDetailPanel
                    ? classes$B.right
                    : isExpanded
                        ? classes$B.up
                        : undefined) })) }) }));
...
const MRT_TableBodyCell = ({ cell, numRows = 1, renderedColumnIndex = 0, renderedRowIndex = 0, rowRef, table, virtualCell, ...rest }) => {
    ...
    return (jsx(TableTd, { "data-column-pinned": isColumnPinned || undefined, "data-dragging-column": isDraggingColumn || undefined, "data-first-right-pinned": (isColumnPinned === "right" &&
            ...
            ...parseFromValuesOrFunc(tableCellProps.style, theme),
                ...
                (columnDef.enableCellHoverReveal ? (jsxs("div", { className: clsx(columnDef.enableCellHoverReveal &&
                        ...
                        classes$z["cell-hover-reveal"], isCellContentOverflowing && classes$z["overflowing"]), ref: cellHoverRevealDivRef, children: [renderCellContent(), cell.getIsGrouped() && !columnDef.GroupedCell && (jsxs(Fragment, { children: [" (", row.subRows?.length, ")"] }))] })) : (jsxs(Fragment, { children: [renderCellContent(), cell.getIsGrouped() && !columnDef.GroupedCell && (jsxs(Fragment, { children: [" (", row.subRows?.length, ")"] }))] }))) }) }));
};
...
const MRT_TableDetailPanel = ({ parentRowRef, renderedRowIndex = 0, row, rowVirtualizer, striped, table, virtualRow, ...rest }) => {
    ...
    return (jsx(TableTr, { "data-index": renderDetailPanel ? renderedRowIndex * 2 + 1 : renderedRowIndex, "data-striped": striped, ref: (node) => {
            ...
            "--mrt-virtual-row-start": virtualRow
                ...
                : undefined,
            ...tableRowProps?.__vars,
        }, className: clsx("mantine-Table-tr-detail-panel", classes$x.root, layoutMode?.startsWith("grid") && classes$x["root-grid"], virtualRow && classes$x["root-virtual-row"], tableRowProps?.className), children: jsx(TableTd, { colSpan: getVisibleLeafColumns().length, component: "td", ...tableCellProps, __vars: {
...
const MRT_TableBodyRow = ({ children, columnVirtualizer, numRows, pinnedRowIds, renderedRowIndex = 0, row, rowVirtualizer, table, tableProps, virtualRow, ...rest }) => {
    ...
    return (jsxs(Fragment, { children: [jsxs(TableTr, { "data-dragging-row": isDraggingRow || undefined, "data-hovered-row-target": isHoveredRow || undefined, "data-index": renderDetailPanel ? renderedRowIndex * 2 : renderedRowIndex, "data-row-pinned": isRowStickyPinned || isRowPinned || undefined, "data-selected": isRowSelected || undefined, "data-striped": striped, onDragEnter: handleDragEnter, ref: (node) => {
                    ...
                    "--mrt-virtual-row-start": virtualRow
                        ? `${virtualRow.start}`
                        : undefined,
                }, className: clsx(classes$y.root, layoutMode?.startsWith("grid") && classes$y["root-grid"], virtualRow && classes$y["root-virtualized"], tableRowProps?.className), children: [virtualPaddingLeft ? (jsx(Box, { component: "td", display: "flex", w: virtualPaddingLeft })) : null, children
                        ? children
...
const MRT_TableBodyEmptyRow = ({ table, tableProps, ...commonRowProps }) => {
    ...
    return (jsxs(MRT_TableBodyRow, { className: clsx("mrt-table-body-row", layoutMode?.startsWith("grid") && classes$C["empty-row-tr-grid"]), table: table, tableProps: tableProps, ...emptyRowProps, children: [renderDetailPanel && (jsx(TableTd, { className: clsx("mrt-table-body-cell", layoutMode?.startsWith("grid") && classes$C["empty-row-td-grid"]), colSpan: 1, children: jsx(MRT_ExpandButton, { row: emptyRow, table: table }) })), jsx("td", { className: clsx("mrt-table-body-cell", layoutMode?.startsWith("grid") && classes$C["empty-row-td-grid"]), colSpan: table.getVisibleLeafColumns().length, children: renderEmptyRowsFallback?.({ table }) ?? (jsx(Text, { __vars: {
...
const MRT_ShowHideColumnsMenuItems = ({ allColumns, column, hoveredColumn, setHoveredColumn, table, }) => {
    ...
    return (jsxs(Fragment, { children: [jsx(Menu.Item, { className: classes$r.root, component: "span", onDragEnter: handleDragEnter, ref: menuItemRef, style: {
                    "--_column-depth": `${(column.depth + 0.5) * 2}rem`,
                    "--_hover-color": getPrimaryColor(theme),
                }, ...dataVariable("dragging", isDragging), ...dataVariable("order-hovered", hoveredColumn?.id === column.id), children: jsxs(Box, { className: classes$r.menu, children: [columnDefType !== "group" &&
                            ...
                            (column.getCanPin() ? (jsx(MRT_ColumnPinningButtons, { column: column, table: table })) : (jsx(Box, { className: classes$r.pin }))), enableHiding ? (jsx(Tooltip, { label: localization.toggleVisibility, openDelay: 1000, withinPortal: true, children: jsx(Switch, { checked: switchChecked, className: classes$r.switch, disabled: !column.getCanHide(), label: columnDef.header, onChange: () => handleToggleColumnHidden(column) }) })) : (jsx(Text, { className: classes$r.header, children: columnDef.header }))] }) }), column.columns?.map((c, i) => (jsx(MRT_ShowHideColumnsMenuItems, { allColumns: allColumns, column: c, hoveredColumn: hoveredColumn, setHoveredColumn: setHoveredColumn, table: table }, `${i}-${c.id}`)))] }));
};
...
const MRT_TableFooterCell = ({ footer, renderedColumnIndex, table, ...rest }) => {
    ...
    return (jsx(TableTh, { colSpan: footer.colSpan, "data-column-pinned": isColumnPinned || undefined, "data-first-right-pinned": (isColumnPinned === "right" &&
            column.getIsFirstColumn(isColumnPinned)) ||
...
const MRT_TableFooterRow = ({ columnVirtualizer, footerGroup, table, ...rest }) => {
    ...
    return (jsxs(TableTr, { className: clsx(classes$o.root, layoutMode?.startsWith("grid") && classes$o["layout-mode-grid"]), ...tableRowProps, children: [virtualPaddingLeft ? (jsx(Box, { component: "th", display: "flex", w: virtualPaddingLeft })) : null, (virtualColumns ?? footerGroup.headers).map((footerOrVirtualFooter, renderedColumnIndex) => {
...
const MRT_TableFooter = ({ columnVirtualizer, table, ...rest }) => {
    ...
    return (jsx(TableTfoot, { ...tableFooterProps, className: clsx(classes$q.root, tableFooterProps?.className, stickFooter && classes$q.sticky, layoutMode?.startsWith("grid") && classes$q.grid), ref: (ref) => {
...
const MRT_ToolbarAlertBanner = ({ stackAlertBanner, table, ...rest }) => {
    ...
    return (jsx(Collapse, { in: showAlertBanner || !!selectedAlert || !!groupedAlert, transitionDuration: stackAlertBanner ? 200 : 0, children: jsx(Alert, { color: "blue", icon: false, ...alertProps, className: clsx(classes$n.alert, stackAlertBanner &&
...
const MRT_FilterCheckbox = ({ column, table, ...rest }) => {
    ...
    return (jsx(Tooltip, { label: checkboxProps?.title ?? filterLabel, openDelay: 1000, withinPortal: true, children: jsx(Checkbox, { checked: value === "true", className: clsx("mrt-filter-checkbox", classes$j.root), indeterminate: value === undefined, label: checkboxProps.title ?? filterLabel, size: density === "xs" ? "sm" : "md", ...checkboxProps, onChange: (e) => {
...
const MRT_FilterRangeFields = ({ header, table, ...rest }) => {
    return (jsxs(Box, { ...rest, className: clsx("mrt-filter-range-fields", classes$i.root, rest.className), children: [jsx(MRT_FilterTextInput, { header: header, rangeFilterIndex: 0, table: table }), jsx(MRT_FilterTextInput, { header: header, rangeFilterIndex: 1, table: table })] }));
};
...
const MRT_FilterRangeSlider = ({ header, table, ...rest }) => {
    ...
    return (jsx(RangeSlider, { className: clsx("mrt-filter-range-slider", classes$g.root), max: max, min: min, onChange: (values) => {
...
const MRT_FilterOptionMenu = ({ header, onSelect, table, }) => {
    ...
    return (jsx(Menu.Dropdown, { children: (header && column && columnDef
            ...
            internalFilterOptions.map(({ divider, label, option, symbol }, index) => (jsxs(Fragment$1, { children: [jsx(Menu.Item, { color: option === filterOption ? "blue" : undefined, leftSection: jsx("span", { className: classes$f.symbol, children: symbol }), onClick: () => handleSelectFilterMode(option), value: option, children: label }), divider && jsx(Menu.Divider, {})] }, index))) }));
};
    ...
    return (jsx(Fragment, { children: jsxs(Popover, { keepMounted: columnDef.filterVariant === "range-slider", onChange: setPopoverOpened, onClose: () => setPopoverOpened(false), opened: popoverOpened, position: "top", shadow: "xl", width: 360, withinPortal: true, children: [jsx(Transition, { mounted: columnFilterDisplayMode === "popover" ||
                        ...
                        (isRangeFilter &&
                            (!!column.getFilterValue()?.[0] ||
                                !!column.getFilterValue()?.[1])), transition: "scale", children: () => (jsx(Popover.Target, { children: jsx(Tooltip, { disabled: popoverOpened, label: filterTooltip, multiline: true, w: filterTooltip.length > 40 ? 300 : undefined, withinPortal: true, children: jsx(ActionIcon, { "aria-label": filterTooltip, className: clsx("mrt-table-head-cell-filter-label-icon", classes$d.root), size: 18, ...dataVariable("active", isFilterActive), onClick: (event) => {
                                    event.stopPropagation();
...
const MRT_TableHeadCellSortLabel = ({ header, table, ...rest }) => {
    ...
    const SortActionButton = (jsx(ActionIcon, { "aria-label": sortTooltip, ...dataVariable("sorted", sorted), ...rest, className: clsx("mrt-table-head-sort-button", classes$b["sort-icon"], rest.className), children: sorted === "desc" ? (jsx(IconSortDescending, { size: "100%" })) : sorted === "asc" ? (jsx(IconSortAscending, { size: "100%" })) : (jsx(IconArrowsSort, { size: "100%" })) }));
...
const MRT_TableHeadCell = ({ columnVirtualizer, header, renderedHeaderIndex = 0, table, ...rest }) => {
    ...
    return (jsxs(TableTh, { colSpan: header.colSpan, "data-column-pinned": isColumnPinned || undefined, "data-dragging-column": isDraggingColumn || undefined, "data-first-right-pinned": (isColumnPinned === "right" &&
            column.getIsFirstColumn(isColumnPinned)) ||
            ...
            ...parseFromValuesOrFunc(tableCellProps?.style, theme),
                ...
                ? null
                : (tableCellProps.children ?? (jsxs(Flex, { className: clsx("mrt-table-head-cell-content", classes$k.content, (columnDefType === "group" ||
                        tableCellProps?.align === "center") &&
                        classes$k["content-center"], tableCellProps?.align === "right" && classes$k["content-right"], column.getCanResize() && classes$k["content-spaced"]), children: [jsxs(Flex, { __vars: {
                                ...
                                : tableCellProps?.align === "center" &&
                                    classes$k["labels-center"], columnDefType === "data" && classes$k["labels-data"]), onClick: column.getToggleSortingHandler(), children: [jsx(Flex, { className: clsx("mrt-table-head-cell-content-wrapper", classes$k["content-wrapper"], columnDefType === "data" &&
                                    ...
                                    (column.getIsSorted() || showColumnButtons) && (jsx(MRT_TableHeadCellSortLabel, { header: header, table: table }))] }), columnDefType !== "group" && (jsxs(Flex, { className: clsx("mrt-table-head-cell-content-actions", classes$k["content-actions"]), children: [showDragHandle && (jsx(MRT_TableHeadCellGrabHandle, { column: column, table: table, tableHeadCellRef: {
...
const MRT_TableHead = ({ columnVirtualizer, table, ...rest }) => {
    ...
    return (jsx(TableThead, { ...tableHeadProps, className: clsx(classes$m.root, layoutMode?.startsWith("grid")
            ? classes$m["root-grid"]
            ...
            (showAlertBanner || getSelectedRowModel().rows.length > 0) ? (jsx(TableTr, { className: clsx(classes$m["banner-tr"], layoutMode?.startsWith("grid") && classes$m.grid), children: jsx(TableTh, { className: clsx(classes$m["banner-th"], layoutMode?.startsWith("grid") && classes$m.grid), colSpan: table.getVisibleLeafColumns().length, children: jsx(MRT_ToolbarAlertBanner, { table: table }) }) })) : (getHeaderGroups().map((headerGroup) => (jsx(MRT_TableHeadRow, { columnVirtualizer: columnVirtualizer, headerGroup: headerGroup, table: table }, headerGroup.id)))) }));
};
...
const createRow = (table, originalRow, rowIndex = -1, depth = 0, subRows, parentId) => createRow$1(table, "mrt-row-create", originalRow ??
...
const MRT_AggregationFns = { ...aggregationFns };
...
const MRT_ProgressBar = ({ isTopToolbar, table, ...rest }) => {
    ...
    return (jsx(Collapse, { className: clsx(classes$7.collapse, isTopToolbar && classes$7["collapse-top"]), in: isSaving || showProgressBars, children: jsx(Progress, { animated: true, "aria-busy": "true", "aria-label": "Loading", radius: 0, value: 100, ...linearProgressProps }) }));
};
...
const MRT_ToolbarInternalButtons = ({ table, ...rest }) => {
    ...
    return (jsx(Flex, { ...rest, className: clsx("mrt-toolbar-internal-buttons", classes$4.root, rest?.className), children: renderToolbarInternalActions?.({ table }) ?? (jsxs(Fragment, { children: [enableFilters &&
                    enableGlobalFilter &&
...
const MRT_TopToolbar = ({ table, ...rest }) => {
    ...
    return (jsxs(Box, { ...toolbarProps, className: clsx(commonClasses["common-toolbar-styles"], classes$3["root"], isFullScreen && classes$3["root-fullscreen"], toolbarProps?.className), ref: (node) => {
            if (node) {
